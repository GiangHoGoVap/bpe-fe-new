...
const Notification = () => {
    ...
    const getUserNotification = async (queryFilter?: IQueryParams) => {
        try {
            const notifications = await notificationApi.getUserNotification({
                ...
                keyword: searchValue,
                page:
                    queryFilter || (searchValue && isSearching)
                        ? 1
                        : pagination.page,
            });
            setPagination({
        ...
        try {
            ...
            if (result) {
                ...
                notify({
                    type: "success",
                    message:
                        "Change status of selected Notification successfully",
                    title: "Success",
                });
            }
        } catch (error) {
            ...
            notify({
                type: "error",
                message:
                    "Something went wrong while star selected Notification",
                title: "Error",
            });
        }
    };
    ...
    const socket = io("https://bpe-be-v2.onrender.com");
    useEffect(() => {
